...
postgresql_restrict_pghba() {
    if [[ -n "$POSTGRESQL_PASSWORD" ]]; then
        replace_in_file "$POSTGRESQL_PGHBA_FILE" "trust" "scram-sha-256" false
        # FIPS 140-3 Compliance: Replace MD5 authentication with SCRAM-SHA-256
        # MD5 is not FIPS-approved and must be replaced with FIPS-compliant methods
        replace_in_file "$POSTGRESQL_PGHBA_FILE" "md5" "scram-sha-256" false
    fi
}
