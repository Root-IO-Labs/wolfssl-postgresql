...
This document describes the entropy and random number generation (RNG) architecture for the PostgreSQL 17.6 FIPS-enabled container image. Proper entropy management is **critical** for FIPS 140-3 compliance, as all security-critical cryptographic operations (key generation, IV creation, nonce generation, etc.) must use approved RNG mechanisms.
...
### 5.1 PostgreSQL Cryptographic Operations

PostgreSQL uses cryptography for:
...
| **pgcrypto Extension** | All crypto via FIPS module | ✅ Via OpenSSL API → wolfProvider |
...
This ensures PostgreSQL links to FIPS OpenSSL, which uses wolfProvider, which uses wolfSSL FIPS DRBG.
...
5. **Source Code Evidence:**
   - PostgreSQL build flags (linking to FIPS OpenSSL)
...
# Test 3: Test PostgreSQL crypto functions
psql -U postgres -c "SELECT encode(gen_random_bytes(32), 'hex');"

# Test 4: Verify no /dev/urandom usage for crypto
strace -e openat postgres 2>&1 | grep urandom
# Should show no urandom access during crypto operations
```
...
# Measure PostgreSQL key generation
time psql -U postgres -c "SELECT gen_random_bytes(1000000);" >/dev/null
```
