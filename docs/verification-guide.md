# FIPS 140-3 Verification Guide for PostgreSQL Container
...
This guide provides comprehensive verification procedures for validating FIPS 140-3 compliance and FedRAMP readiness of the PostgreSQL 17.6 FIPS-enabled container image. This document is designed for:
...
- [ ] Container image built successfully: `postgresql-fips-ubuntu:17.6.0`
...
docker run --rm --entrypoint /bin/bash postgresql-fips-ubuntu:17.6.0 \
...
#### Test 3.4.3: Verify libssl3 Package Removal
...
**Expected Output:**
```
(no libssl3 package listed, or marked as removed)
```

**Pass Criteria:**
- ✓ libssl3 not installed or marked as removed
...
docker run --rm postgresql-fips-ubuntu:17.6.0 openssl version

# Detailed version info
docker run --rm postgresql-fips-ubuntu:17.6.0 openssl version -a
```
...
docker run --rm postgresql-fips-ubuntu:17.6.0 openssl list -providers

# Verbose provider info
docker run --rm postgresql-fips-ubuntu:17.6.0 openssl list -providers -verbose
```
...
docker run --rm postgresql-fips-ubuntu:17.6.0 openssl rand -hex 32
...
echo "sensitive data" | docker run --rm -i postgresql-fips-ubuntu:17.6.0 \
...
## 4. PostgreSQL-Specific Verification

### 4.1 PostgreSQL Build Verification

**Objective:** Confirm PostgreSQL is built with FIPS OpenSSL

#### Test 4.1.1: PostgreSQL Version

```bash
# Check PostgreSQL version
docker run --rm postgresql-fips-ubuntu:17.6.0 postgres --version
```
...
#### Test 4.1.2: PostgreSQL SSL Linkage

```bash
# Verify PostgreSQL links to FIPS OpenSSL
docker run --rm postgresql-fips-ubuntu:17.6.0 \
  ldd /opt/bitnami/postgresql/bin/postgres | grep -E "ssl|crypto"
```
...
#### Test 4.1.3: PostgreSQL Build Configuration

```bash
# Check PostgreSQL was built with SSL support
docker run --rm postgresql-fips-ubuntu:17.6.0 \
  bash -c "strings /opt/bitnami/postgresql/bin/postgres | grep -i 'OpenSSL' | head -5"
```
...
### 4.2 PostgreSQL Runtime Verification

**Objective:** Verify PostgreSQL operates correctly with FIPS crypto

#### Test 4.2.1: Container Startup Test

```bash
# Start PostgreSQL container
docker run -d --name test-pg-fips \
...
# Check status
docker ps | grep test-pg-fips

# Check logs for FIPS validation
docker logs test-pg-fips 2>&1 | grep "FIPS VALIDATION"
```
...
**Cleanup:**
```bash
docker stop test-pg-fips && docker rm test-pg-fips
```

---

#### Test 4.2.2: PostgreSQL SSL Configuration
...
# Check SSL setting
docker exec test-pg-fips psql -U postgres -c "SHOW ssl;"
...
#### Test 4.2.3: pgcrypto Extension with FIPS

```bash
# Install pgcrypto extension
docker exec test-pg-fips \
  psql -U postgres -c "CREATE EXTENSION IF NOT EXISTS pgcrypto;"

# Test SHA-256 via pgcrypto
docker exec test-pg-fips \
  psql -U postgres -c "SELECT encode(digest('test', 'sha256'), 'hex');"

# Expected: 9f86d081884c7d659a2feaa0c55ad015a3bf4f1b2b0b822cd15d6c15b0f00a08

# Test random number generation
docker exec test-pg-fips \
  psql -U postgres -c "SELECT encode(gen_random_bytes(16), 'hex');"

# Should output 32 hex characters
```

**Pass Criteria:**
- ✓ pgcrypto loads successfully
- ✓ SHA-256 produces correct hash
- ✓ Random generation works
- ✓ All operations use FIPS crypto (via OpenSSL → wolfProvider → wolfSSL)

**Evidence to Collect:**
- pgcrypto test results
- Hash verification
- Random output samples

---

#### Test 4.2.4: PostgreSQL Connection Test

```bash
# Test client connection
docker exec test-pg-fips pg_isready -U postgres
...
ls -lh postgresql/17.6.0-ubuntu-22.04/hardening/ubuntu-22.04-stig.sh

# Review script contents
head -50 postgresql/17.6.0-ubuntu-22.04/hardening/ubuntu-22.04-stig.sh
```
...
docker run --rm postgresql-fips-ubuntu:17.6.0 ls -la /var/log/
...
docker exec test-pg-logs ls -la /opt/bitnami/postgresql/logs/ 2>/dev/null || \
docker exec test-pg-logs ls -la /bitnami/postgresql/logs/ 2>/dev/null
...
PostgreSQL 17.6 FIPS Container
...
./tests/quick-test.sh postgresql-fips-ubuntu:17.6.0
```
...
docker run --rm --entrypoint bash postgresql-fips-ubuntu:17.6.0 \
  -c "find /usr/lib /lib -name 'libssl.so*' 2>/dev/null"

# Verify PostgreSQL linkage
docker run --rm postgresql-fips-ubuntu:17.6.0 \
  ldd /opt/bitnami/postgresql/bin/postgres | grep ssl

# Full entrypoint validation
docker run --rm postgresql-fips-ubuntu:17.6.0 \
  /usr/local/bin/fips-entrypoint.sh postgres --version
...
**Next Review:** Upon PostgreSQL version update or FIPS module change
