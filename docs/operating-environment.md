...
This document defines the Operating Environment (OE) for the PostgreSQL 17.6 FIPS-enabled container image built on Ubuntu 22.04. The OE specification is critical for FIPS 140-3 CMVP compliance, as cryptographic module validation is tied to specific operating system versions, kernel versions, and CPU architectures.
...
| **AES-NI** | ⚠️ Recommended | Hardware-accelerated AES |
...
```bash
# Validation Steps
[1] Check kernel version against allowed range
[2] Check CPU architecture (must be x86_64)
[3] Check for required CPU features (RDRAND recommended)
[4] Verify glibc version compatibility
[5] Check that wolfSSL FIPS module is properly installed
[6] Verify Operating Environment consistency
```
...
[1/4] Kernel Version
      ✓ Detected: Linux 6.8.0-31-generic
      ✓ Validated: Within wolfSSL CMVP OE range (6.8.x - 6.14.x)

[2/4] CPU Architecture
      ✓ Detected: x86_64
      ✓ Validated: Matches CMVP certificate requirement
...
[4/4] System Libraries
      ✓ glibc: 2.39 (compatible)
      ✓ wolfSSL: 5.8.2 FIPS v5.2.3

========================================
✓ OPERATING ENVIRONMENT VALIDATED
========================================
```
...
6. **wolfSSL Validation Letter** (optional, from consulting engagement)
...
- ❌ Ubuntu versions other than 22.04 with this image
...
3. **Implement Runtime OE Validation**
   - Update `fips-entrypoint.sh` with kernel version checks
   - Add CPU architecture validation
   - Add CPU feature detection (RDRAND)
...
|---------|------|--------|---------|
| 1.0 | 2025-12-03 | Claude Code | Initial draft based on remediation plan |

---

**Document Status:** DRAFT - Requires wolfSSL CMVP certificate validation before finalization

**Next Review Date:** Upon receipt of wolfSSL CMVP certificate
